...
static inline int	malloc_init_philo(t_fork **r_fork, t_philo *philo)
{
	if (!(*r_fork = m(sizeof(t_fork))) ||
		!((*r_fork)->fork = m(sizeof(pthread_mutex_t))) ||
		!(philo->thread = m(sizeof(pthread_t))) ||
		!(philo->monitor = m(sizeof(pthread_t))) ||
...
int					init_philo(t_one *one)
{
	...
	while (++i < one->param->number_of_philosophers)
	{
		if (malloc_init_philo(&r_fork, &(one->philo[i])) < 0)
			return (ERROR_MALLOC);
		one->philo[i].num = i + 1;
		...
		*one->philo[i].end = 0;
		if (pthread_mutex_init(r_fork->fork, NULL))
			return (ERROR_MUTEX);
		one->philo[i].right_fork = r_fork;
		if (i > 0)
			one->philo[i].left_fork = one->philo[i - 1].right_fork;
	}
	one->philo[0].left_fork = one->philo[i - 1].right_fork;
	return (0);
}
